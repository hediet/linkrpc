...
import type { Transport } from '@vscode/hubrpc/hub/common';
...
function makeAttested(
...
): { transport: AttestedTransport; peer: HubRpcConnection; } {
    ...
    return { transport, peer: HubRpcConnection.fromTransport(pair.b) };
}
...
function attachProvider(hub: Hub, serviceId: string): HubRpcConnection {
    const pair = new TransportPair();
    hub.attach(pair.a);
