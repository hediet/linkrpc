...
import { KeypairSigningIdentity } from '@vscode/hubrpc';
import {
...
describe('withForwardedCallGate (hub signature front door)', () => {
    it('marks accepted reserved inspection calls but not similarly named methods', async () => {
        ...
        expect(transits).toEqual([visibleMethod, visibleMethod]);
        observation.dispose();
    });
    ...
    it('keeps the envelope intact through chained gates and strips it only for a strict terminal handler', async () => {
        ...
        const server = HubRpcConnection.fromTransport(secondGate);
...
describe('capability demo flow', () => {
    it('gates fully-qualified calls before they reach service handlers after a JSON round-trip', async () => {
        ...
        const server = HubRpcConnection.fromTransport(gatedTransport);
        let handlerCalls = 0;
