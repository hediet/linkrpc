...
import { defineInterface, requestType, HubRpcConnection } from '@vscode/hubrpc';
import { z } from 'zod';
import { connectNdjson } from '@vscode/hubrpc/node';
    ...
    it('accepts a connection and round-trips a call', async () => {
        ...
        try {
            server.setConnectionHandler((transport) => {
                const conn = HubRpcConnection.fromTransport(transport);
