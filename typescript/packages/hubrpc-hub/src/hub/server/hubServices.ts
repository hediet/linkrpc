...
import type { StreamApi } from '@vscode/hubrpc';
import {
    topologyInterface,
    trafficInterface,
    type TrafficEvent,
...
export interface HubServices {
    ...
    readonly connection: HubRpcConnection<RegisterCallContext>;
...
export function createHubServiceInterfaces(hub: Hub, options: HubServicesOptions = {}): HubServices {
    ...
    const connection = new HubRpcConnection<RegisterCallContext>(baseChannel);
...
function registerInspectionInterfaces(
    connection: HubRpcConnection<RegisterCallContext>,
    hub: Hub,
