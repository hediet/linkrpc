...
/**
 * Build a splitter wired to three HubRpcConnections — one per port — so we
...
function makeRig() {
    ...
    return {
        splitter,
        participant: HubRpcConnection.fromTransport(pPair.b),
        root: HubRpcConnection.fromTransport(cPair.b),
        uplink: HubRpcConnection.fromTransport(hPair.b),
    };
}
