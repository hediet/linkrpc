...
import { z } from 'zod';
import { directoryInterface, directoryWatchNever } from '@vscode/hubrpc';
import { hubGrantedServiceIdInterface, walkHub } from '@vscode/hubrpc/hub/common';
...
async function joinParticipant(
    hub: Hub,
    prefix: string,
): Promise<{ conn: HubRpcConnection; overlay: RootOverlay; upstream: AttachedLink; }> {
...
function attachConsumer(hub: Hub): HubRpcConnection {
    const pair = new TransportPair();
    hub.attach(pair.a);
...
describe('RootOverlay + hub services (end-to-end)', () => {
    ...
    it('discovers public root -> cloud -> Auth through explicit directory listings', async () => {
        ...
        const cloud = HubRpcConnection.fromTransport(cloudPair.b);
    ...
    it('ticks the global directory when a participant adds a referral under an existing claim', async () => {
        ...
        const pair = new TransportPair();
        const child = hub.attach(pair.a);
        const participant = HubRpcConnection.fromTransport(pair.b);
    ...
    it('the walk folds transitive rootPrincipalSets from a referral onto descendants', async () => {
        ...
        const parent = HubRpcConnection.fromTransport(parentPair.b);
        ...
        const child = HubRpcConnection.fromTransport(childPair.b);
    ...
    it('hubs nest: a child hub uplinks to a parent and reaches its services', async () => {
        ...
        const osConn = HubRpcConnection.fromTransport(svcPair.b);
