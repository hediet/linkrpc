#!/usr/bin/env node
/**
 * `hubrpc-hub` — run a hub from a declarative config file. This is the standalone
...
async function main(argv: readonly string[]): Promise<void> {
    ...
    program
        .name('hubrpc-hub')
        .description(
            'Run a hubrpc hub from a declarative config file. The config format is shared with '
            + 'the `hub`/`hubrpc` CLI (`serve`, `-c`) and the VS Code extension.',
        )
        ...
        .action(async (config: string | undefined, opts: { printSchema: boolean; cmdInteractive: boolean; }) => {
            ...
            await serveHub({
                ...
                log: (line) => console.log(`[hubrpc-hub] ${line}`),
            });
        });

    await program.parseAsync(['node', 'hubrpc-hub', ...argv]);
}
