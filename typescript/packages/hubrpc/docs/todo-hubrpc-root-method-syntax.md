...
`hubrpc/src/protocol/methodName.ts` — `parseMethodName`. Single source of truth.
...
- [ ] `hubrpc/src/connection/hubRpcConnection.ts` `_buildClient` (~L270): root branch
      `${iface.info.id}::${name}` → `::${iface.info.id}::${name}`.
- [ ] `hubrpc-cli/src/methodRef.ts` `getMethodOnWire` (~L52): interface branch →
      `::${interfaceId}::${methodName}`.
- [ ] `hubrpc-cli/src/ui/UiModel.ts` `peekPendingCall` + `submit` (~L320): root
      branches → `::${interfaceId}::${methodName}`.
- [ ] `hubrpc-cli/src/reflection.ts` (~L71): non-target branches
      `"hubrpc.directory::list"` / `"hubrpc.schemas::get"` →
      `"::hubrpc.directory::list"` / `"::hubrpc.schemas::get"`.
- [ ] `hubrpc-cli/src/hubSigning.ts` `_resolveHubAccessMethod` (~L68): `bare` →
...
- [ ] `hubrpc-cli/src/commands/tunnel.ts` `CLAIM_METHOD` (~L11) → `::hubParticipant::register`.
- [ ] `hubrpc-hub/src/engine/runHub.ts` `REGISTER_METHOD` (~L78) → `::hubParticipant::register`.
- [ ] `hubrpc-cli/src/suggest.ts` (~L176) and completion builders in
      `hubrpc-cli/src/completions/complete.ts` (~L194) — emit root methods as `::iface::method`.
...
- [ ] `hubrpc-mcp/src/server.ts` (~L336) `isForm3 = method.split("::").length === 3`
      → `parseMethodName(method)?.kind === "full"`.
- [ ] `hubrpc-cli/src/hubSigning.ts` (~L165) `wireMethod.split('::')` — re-derive via parse.
- [ ] `hubrpc-cli/src/completions/complete.ts` (~L169) `_completeMethodRef`
      separator-count logic — reshape so leading `::` drives root-interface completion.

## 4. `$stream::send` wire constant

- [ ] `hubrpc/src/streaming.ts` (~L141) `STREAM_METHOD = "$stream::send"` →
      `::$stream::send`. Matched exactly in:
  - `hubrpc/src/hub/server/routingHub.ts` (~L400)
  - `hubrpc/src/connection/jsonRpcChannel.ts` (~L192, L235)
  - `hubrpc/src/hub/server/overlaySplitter.ts` (~L11)
  - Only the constant + comments/tests need touching (comparisons use the constant).

## 5. CLI input parsing & back-compat

- [ ] `hubrpc-cli/src/methodRef.ts` `parseMethodRef` (~L9): accept leading `::`
      (empty first segment) as explicit root form.
- [ ] `hubrpc-cli/src/completions/complete.ts` (~L260) core/parts handling.
...
- [ ] `hubrpc-cli/src/methodRef.test.ts`
- [ ] `hubrpc-cli/src/completions/complete.test.ts`
- [ ] `hubrpc-mcp/src/taskRegistry.test.ts` (`svc::iface::m`, `a::b::c`)
- [ ] `hubrpc/src/identity/metaEnvelope.test.ts`
- [ ] `hubrpc/src/hub/server/forwardedCallGate.test.ts`
- [ ] `hubrpc/src/hub/server/overlaySplitter.test.ts`
- [ ] `hubrpc/src/connection/hubRpcConnection.test.ts`
- [ ] `methodName` tests
- [ ] `hubrpc/scripts/smoke-hub.ts`: `hubDirectory::listPrefixes` → `::hubDirectory::listPrefixes`
- [ ] Doc-comment occurrences of `hubAccess::…`, `hubParticipant::register`,
      `hubInfo::whoami`, `$stream::send`, `scriptRunner::run`, etc. across
      `hubrpc-node-runner`, `cliConsent.ts`, `config.ts`, READMEs (cosmetic).

## 7. Verification

- [ ] `pnpm exec vitest run` in `hubrpc/hubrpc` and the CLI/MCP packages.
