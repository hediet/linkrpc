...
import type { MethodSchema, HubRpcInterfaceSchema } from '../schema/hubRpcInterfaceSchema';
import type { HubRpcJsonSchema } from '../schema/hubRpcJsonSchema';
 ...
 * `HubRpcConnection.register`. Request handlers may return synchronously or
 * via a promise; notification handlers return void.
 *
 * `TCtx` is the call-context type carried by the hosting connection — see
 * `HubRpcConnection<TCtx>`. For the default connection (`TCtx = undefined`)
 ...
 * `HubRpcInterfaceSchema` (e.g. produced by codegen or, in the faker, by an
...
export interface InterfaceDefinitionOpts {
    frozenSchema?: HubRpcInterfaceSchema;
}

interface InterfaceDefinitionState {
    readonly frozenSchema: HubRpcInterfaceSchema | undefined;
    schemaCache: HubRpcInterfaceSchema | undefined;
...
export class InterfaceDefinition<TMembers extends MemberMap> {
    ...
    public toSchema(): HubRpcInterfaceSchema {
        const state = interfaceDefinitionState.get(this)!;
...
function buildSchema(
    ...
    const components: Record<string, HubRpcJsonSchema> = {};
    for (const [name, member] of Object.entries(members)) {
        methods[name] = toMethodSchema(name, member, components);
    }
    const schema: HubRpcInterfaceSchema = {
        id: info.id,
        hash,
        methods,
    };
 ...
 * schemas attached.
 */
export function interfaceFromSchema(schema: HubRpcInterfaceSchema): InterfaceDefinition<MemberMap> {
