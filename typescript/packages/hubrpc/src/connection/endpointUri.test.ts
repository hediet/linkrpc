...
describe('parseEndpointUri', () => {
    ...
    it('parses npipe: into a Windows pipe path', () => {
        expect(parseEndpointUri('npipe://./pipe/hubrpc?token=t')).toEqual({
            kind: 'socket',
            path: '\\\\.\\pipe\\hubrpc',
            token: 't',
        });
    ...
    it('parses broker mode on a socket endpoint', () => {
        expect(parseEndpointUri('npipe://./pipe/hubrpc?token=t&broker=raw')).toEqual({
            kind: 'socket',
            path: '\\\\.\\pipe\\hubrpc',
            token: 't',
            brokerMode: 'raw',
...
describe('formatEndpointUri', () => {
    ...
    it('formats a Windows pipe back to npipe:', () => {
        expect(formatEndpointUri({ kind: 'socket', path: '\\\\.\\pipe\\hubrpc', token: 't' }, { revealToken: true }))
            .toBe('npipe://./pipe/hubrpc?token=t');
    });
    ...
    it('formats broker mode on a socket endpoint', () => {
        expect(formatEndpointUri({
            ...
            token: 't',
            brokerMode: 'raw',
        }, { revealToken: true })).toBe('npipe://./pipe/hubrpc?token=t&broker=raw');
    });
    ...
    it('round-trips through parse with revealToken', () => {
        const cases: ResolvedEndpoint[] = [
            ...
            { kind: 'socket', path: '\\\\.\\pipe\\hubrpc', token: 't' },
