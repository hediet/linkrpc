...
import { stripHubRpcWireMeta } from '../protocol/hubRpcEnvelope';
import {
    defaultsInterface,
    directoryInterface,
    schemasInterface,
...
export interface HubRpcConnectionOptions {
    /**
 ...
 */
export class HubRpcConnection<TInCtx = any, TOutCtx = any> {
    /**
    ...
    public static fromTransport<TInCtx = undefined>(
        ...
        options: HubRpcConnectionOptions = {},
    ): HubRpcConnection<TInCtx> {
        return new HubRpcConnection<TInCtx>(JsonRpcChannel.create<TInCtx>(transport), options);
    }
    ...
    private async _handleRequest(call: IncomingCall<TInCtx>): Promise<Result> {
        ...
        const userParams = stripHubRpcWireMeta(call.params);
