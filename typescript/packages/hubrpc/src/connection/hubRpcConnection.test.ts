...
import { HubRpcConnection } from './hubRpcConnection';
...
function makePair(): { client: HubRpcConnection; server: HubRpcConnection; dispose: () => void; } {
    const pair = new TransportPair();
    const client = HubRpcConnection.fromTransport(pair.a);
...
describe('HubRpcConnection', () => {
    ...
    it('can disable outbound request param validation', async () => {
        const pair = new TransportPair();
        const client = HubRpcConnection.fromTransport(pair.a, { validateOutboundParams: false });
        const server = HubRpcConnection.fromTransport(pair.b);
        server.register(greeter, {
            hello: async ({ name }) => ({ greeting: `Hi, ${name}!` }),
            shout: () => { },
...
describe('HubRpcConnection — preset (form 1)', () => {
...
describe('HubRpcConnection — reflection', () => {
