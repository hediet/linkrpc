...
export interface GenerateInterfaceOptions {
     ...
     * `notificationType` are imported. Defaults to `@vscode/hubrpc`.
     */
    hubRpcImport?: string;
    /**
     ...
     * (e.g. `hubrpc.directory` → `hubRpcDirectoryInterface`).
     */
    exportName?: string;
    /**
...
export function generateTsInterface(
    ...
    const definitionImport = preserveWireSchema ?
        'InterfaceDefinition, notificationType, requestType, type HubRpcInterfaceSchema' :
        'defineInterface, notificationType, requestType';
    w.writeLine(`import { ${definitionImport} } from "${hubRpcImport}";`);
    ...
    if (preserveWireSchema) {
        w.writeLine(
            `const wireSchema: HubRpcInterfaceSchema = ${JSON.stringify(schema)};`,
        );
        w.writeLine();
    }
...
function _writeMembers(
    ...
    componentSchemas: Record<string, HubRpcJsonSchema>,
    preserveWireSchema: boolean,
): void {
    w.append('{').newline();
    w.indent();
...
function _writeSchema(
    ...
    if (Array.isArray(s['oneOf'])) {
        const branches = s['oneOf'] as HubRpcJsonSchema[];
    ...
    // array
    if (s['type'] === 'array') {
        const items = (s['items'] as HubRpcJsonSchema | undefined) ?? true;
        w.append('z.array(');
...
function _writeObject(
    ...
    const properties = (s['properties'] as Record<string, HubRpcJsonSchema> | undefined) ?? {};
    ...
    if (additional !== false && additional !== undefined && additional !== true) {
        w.append('.catchall(');
        _writeSchema(w, additional as HubRpcJsonSchema, components, preserveWireSchema, currentComponent);
        w.append(')');
    }
}
...
function _writeTuple(
    ...
    const prefix = s['prefixItems'] as HubRpcJsonSchema[];
    ...
    if (rest !== undefined && rest !== false) {
        w.append('.rest(');
        _writeSchema(w, rest as HubRpcJsonSchema, components, preserveWireSchema, currentComponent);
        w.append(')');
    }
}
...
function _methodDescription(
    method: MethodSchema,
    components: Record<string, HubRpcJsonSchema>,
): string | undefined {
...
function _schemaDescription(schema: HubRpcJsonSchema | undefined): string | undefined {
...
): [string, HubRpcJsonSchema][] {
    const result: [string, HubRpcJsonSchema][] = [];
...
function _containsRef(schema: HubRpcJsonSchema, name: string): boolean {
