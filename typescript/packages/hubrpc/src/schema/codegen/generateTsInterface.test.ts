...
import type { HubRpcInterfaceSchema } from "../hubRpcInterfaceSchema";
import {
    defaultsInterface,
    directoryInterface,
    schemasInterface,
...
async function _roundTrip(def: { toSchema(): HubRpcInterfaceSchema; schemaHash: string }): Promise<void> {
    ...
    const canonical = (s: HubRpcInterfaceSchema) => ({ ...s, hash: "" });
...
describe("generateInterface", () => {
    ...
    it("produces stable, snapshot-able output for directoryInterface", () => {
        ...
        expect(generateTsInterface(directoryInterface.toSchema())).toMatchInlineSnapshot(`
          "import { defineInterface, notificationType, requestType } from "@vscode/hubrpc";
          import { z } from "zod";
