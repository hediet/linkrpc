...
describe("isAssignable — arrays & tuples", () => {
    ...
    it("tuple <: tuple element-wise", () => {
        const sub: HubRpcJsonSchema = { type: "array", prefixItems: [{ type: "integer" }, { type: "string" }], items: false };
        const sup: HubRpcJsonSchema = { type: "array", prefixItems: [{ type: "number" }, { type: "string" }], items: false };
        expect(isAssignable(sub, sup)).toBe(true);
    });
    it("bounded tuple <: unbounded array of widening type", () => {
        const sub: HubRpcJsonSchema = { type: "array", prefixItems: [{ type: "integer" }], items: false };
        const sup: HubRpcJsonSchema = { type: "array", items: { type: "number" } };
        expect(isAssignable(sub, sup)).toBe(true);
    });
});

describe("isAssignable — objects", () => {
    const open = (props: Record<string, HubRpcJsonSchema>, req?: string[]): HubRpcJsonSchema => ({
        type: "object", properties: props, required: req, additionalProperties: true,
    });
    const closed = (props: Record<string, HubRpcJsonSchema>, req?: string[]): HubRpcJsonSchema => ({
...
describe("isAssignable — refs & cycles", () => {
    it("resolves $ref", () => {
        const components = {
            schemas: {
                Foo: { type: "object", properties: { n: { type: "integer" } }, required: ["n"], additionalProperties: false } as HubRpcJsonSchema,
            },
        };
        expect(isAssignable(
