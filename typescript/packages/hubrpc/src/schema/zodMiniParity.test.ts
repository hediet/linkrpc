...
/**
 * Parity between classic `zod` and `zod/mini` for the purpose of hubrpc's wire
 * schema + interface hash.
 *
 * Background: hubrpc derives every interface's JSON schema (and therefore its
...
describe('zod mini / classic parity for hubrpc schema hashing', () => {
    for (const c of cases) {
        it(`${c.name}: classic-authored and mini-authored produce identical hubrpc JSON`, () => {
...
/**
 * Processor parity: hubrpc currently calls classic `z.toJSONSchema`. To drop
