...
/**
 * A minimal, real-world 1:1 hubrpc app: an interface, a server that serves it
...
/**
 * Mirror hubrpc's published `"sideEffects": false` (package.json) onto its own
...
function groupOf(id: string): string {
    ...
    if (nm === -1) {
        if (m.includes('/src/')) return 'src (hubrpc)';
...
describe('bundle size', () => {
    it('a 1:1 stdio client+server bundles under the size budget', async () => {
        ...
        // `zod/mini` and hubrpc's internals do too, so only zod's shared *core*
