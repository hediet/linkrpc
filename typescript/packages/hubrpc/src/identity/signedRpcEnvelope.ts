/**
 * Sign / verify the hubrpc identity envelopes on a JSON-RPC call.
 *
...
import {
    ...
    type CallMeta,
    type HubRpcUnsigned,
    type HubRpcWireParams,
    requireObjectParams,
    stripHubRpcWireMeta,
} from '../protocol/hubRpcEnvelope';
...
export interface SignedRpcCall {
     ...
     */
    readonly wireParams: HubRpcWireParams;
...
export async function signRpcCall(opts: SignRpcCallOptions): Promise<SignedRpcCall> {
    ...
    const wireParams = await signObject("call", signedParams, opts.signingIdentity) as HubRpcWireParams;
...
export function attachCapabilities(
    ...
    const existing = obj[HUBRPC_UNSIGNED_KEY] as HubRpcUnsigned | undefined;
    const merged: HubRpcUnsigned = { ...existing, capabilities };
    return { ...obj, [HUBRPC_UNSIGNED_KEY]: merged } as HubRpcWireParams;
}
...
export async function verifyRpcCall(opts: VerifyRpcCallOptions): Promise<VerifyRpcCallResult> {
    ...
    const userParams = stripHubRpcWireMeta(wireParams);
    ...
    if (
        ...
        return { ok: false, reason: 'malformed hubrpc envelope' };
    }
    ...
    const unsigned = _extractObject<HubRpcUnsigned>(wireParams, HUBRPC_UNSIGNED_KEY);
