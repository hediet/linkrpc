...
hubrpc is a **specialization of [JSON-RPC 2.0](https://www.jsonrpc.org/specification)**: every
message on the wire is a valid JSON-RPC message, and hubrpc adds just enough on top to let many
...
sequenceDiagram
    ...
    participant Caller as Consumer HubRPC node
    participant Hub as Hub (optional intermediary)
    participant Provider as Provider HubRPC node
...
```ts
// shared.ts
import { defineInterface, requestType, notificationType } from '@vscode/hubrpc';
import { z } from 'zod';
...
import { WebSocketServer } from '@vscode/hubrpc-hub/hub/server/node';
import { greeter } from './shared.js';

const token = process.env.HUBRPC_TOKEN;
if (!token) throw new Error('HUBRPC_TOKEN is required');
...
const client = HubRpcConnection.fromTransport(transport);
...
backed by its live registry, so a peer (or the [`hubrpc` CLI](../hubrpc-cli)) can explore and call
...
and *what they're allowed to do*. hubrpc layers both on top of the same JSON-RPC envelope, and both
...
```ts
import { HubRpcConnection } from '@vscode/hubrpc';
import { withFullyQualifiedCallGate } from '@vscode/hubrpc-hub/hub/server';

const serverTransport = withFullyQualifiedCallGate(transport.b, {
    requireCapability: true,
    trustedRoots: [adminId.publicSigningIdentity],
});
const server = HubRpcConnection.fromTransport(serverTransport);
let handlerCalls = 0;
...
const client = new HubRpcConnection(signedChannel);
...
This is what lets a [hub](../hubrpc-hub) broker calls between mutually-distrusting participants: it
hands each consumer a scoped capability and enforces it on every routed call.

## A layered protocol

hubrpc is built in additive layers. Lower layers stand alone; higher ones ride in reserved
...
[`hubrpc-spec`](../hubrpc-spec) (start at [`00-overview.md`](../hubrpc-spec/00-overview.md)). The
...
The core (`@vscode/hubrpc`) is environment-agnostic. Platform transports live behind subpath
exports so browser bundles never pull in Node built-ins:

| Import | Provides |
|---|---|
| `@vscode/hubrpc` | interfaces, connection, schema/hash, identity, capabilities |
| `@vscode/hubrpc/node` | NDJSON sockets, WebSocket, stdio, `connectToHub`, endpoint-URI parsing |
| `@vscode/hubrpc/web` | `WindowMessageTransport` (iframe / web worker) |
| `@vscode/hubrpc/hub/common`, `/hub/client` | hub-facing interfaces and client helpers |

On Node, a single call resolves an endpoint from the environment and dials it:

```ts
import { connectToHub } from '@vscode/hubrpc/node';
import { directoryInterface } from '@vscode/hubrpc';

// reads HUBRPC_ENDPOINT (unix:/npipe:/ws:/wss:/cmd:) and HUBRPC_TOKEN
...
| Package | Role |
|---|---|
| **`@vscode/hubrpc`** (this package) | the core library |
| [`@vscode/hubrpc-cli`](../hubrpc-cli) | generic CLI / terminal UI for any endpoint |
| [`@vscode/hubrpc-hub`](../hubrpc-hub) | standalone WebSocket hub that routes between participants |

## Development

```sh
pnpm --filter @vscode/hubrpc build
pnpm --filter @vscode/hubrpc test
```
