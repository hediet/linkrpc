...
interfaces (`linkrpc.defaults`, `linkrpc.directory`, `linkrpc.schemas`) are
...
| `LINKRPC_ENDPOINT` (env)          | A URI, or a legacy bare socket path / `ws://` URL. Token from `LINKRPC_TOKEN`.                              |

`--endpoint-token <token>` overrides the URI token / `LINKRPC_TOKEN` for LinkRPC
...
| `ws://host:7700?token=…` / `wss://…`      | LinkRPC WebSocket (token → `linkrpc::initialize`) |
...
linkrpc call acme.mailer::email::send --param to=a@b.c --endpoint wss://hub.example.com --endpoint-token abc
...
| `defaults`                    | Print the preset service/interface (wraps `linkrpc.defaults::get`).                                   |
| `schema <interfaceId>[@hash]` | Print an interface schema (wraps `linkrpc.schemas::get`). `--method <name>`, `--json`.                |
...
`linkrpc.directory`. Repeating `--source` queries exactly those providers and
...
| `connect [--timeout 30s] [--ttl 5min] [--schema source]` | Open the configured remote once in a detached broker and print its authenticated local `unix:` / `npipe:` endpoint. `--timeout` is transport inactivity; `--ttl` is an absolute lifetime. `--schema` overlays static LinkRPC reflection from a local JSON file or an HTTP(S) URL. |
| `connection-status`                                   | Read lifecycle and buffer status from the broker configured by `--endpoint` / `LINKRPC_ENDPOINT`. |
...
secret. The broker handles `linkrpc.connectionBroker::status`,
`linkrpc.connectionBroker::readNotifications`, and
`linkrpc.connectionBroker::disconnect` locally. With `--schema`, it also handles
`linkrpc.directory`, `linkrpc.schemas`, and `linkrpc.defaults` from the static
...
including `linkrpc::initialize`. Output goes to stderr. This is lower-level than
