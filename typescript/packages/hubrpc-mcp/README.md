...
Stdio MCP server that lets an LLM drive a [hubrpc](../hubrpc) hub.
...
| `lastResultVal`          | The previous `runHubRpcScript` result against the same hub. JSON round-tripped. `undefined` on the first call.                                                                                      |
...
Set `connection` to a hub endpoint URI (e.g. `unix:/path?token=…`, `npipe://./pipe/…?token=…`, or `wss://host?token=…`) to target a specific hub, or omit it to use the `HUBRPC_ENDPOINT` / `HUBRPC_TOKEN` env vars (set by the team-tools extension on every terminal). The token may be embedded in the URI or supplied via `HUBRPC_TOKEN`.
...
`presentation: "raw"` on `runHubRpcScript`, `awaitHubRpcTask`, or
`cancelHubRpcTask` to expose the original JSON, or selectively return
`mcp.raw(value)`.
...
### `hubrpc-mcp://docs/connection.d.ts`

TypeScript declarations for the in-sandbox API (`con`, `console`, `lastResultVal`). Read this once before authoring `runHubRpcScript` calls.
