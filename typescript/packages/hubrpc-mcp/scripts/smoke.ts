 ...
 */
import { InMemoryManagedIdentity, type ManagedIdentity } from "@vscode/hubrpc";
import { Hub, HubConnectionAcceptor, createHubServiceInterfaces } from "@vscode/hubrpc-hub/hub/server";
import { SocketServer } from "@vscode/hubrpc-hub/hub/server/node";
import { Ed25519CryptoProvider, formatEndpointUri } from "@vscode/hubrpc/node";
        ...
        const host: SandboxHostApi = {
            ...
            requestAccess: async (argsJson) => {
                ...
                const result = await pooled.session.requestAccess({
                    consumer: { name: "hubrpc-mcp", purpose: args.purpose },
