 ...
 */
describe("HubRpcMcpServer e2e (con.explore against a gated hub)", () => {
    it("explore() sees the root interfaces and reports the hub directory as gated", async ({ onTestFinished }) => {
        ...
        // hubrpc.directory, hubrpc.schemas) is visible without any capability;
        ...
        const res = await client.callTool({ name: "runHubRpcScript", arguments: { code: `async ({ con }) => con.explore()` } });
    ...
    it("explore({ requestPermission: true }) unlocks the gated hub directory and reveals the service", async ({ onTestFinished }) => {
        ...
        // With requestPermission the walk requests a reflection cap (hubrpc.* on
        ...
        // under `inaccessible`.
        const res = await client.callTool({ name: "runHubRpcScript", arguments: { code: `async ({ con }) => con.explore({ requestPermission: true, maxResults: 0 })` } });
    ...
    it("filters by serviceId, returning only that service's interfaces", async ({ onTestFinished }) => {
        ...
        const res = await client.callTool({ name: "runHubRpcScript", arguments: { code: `async ({ con }) => con.explore({ requestPermission: true, serviceId: "hello" })` } });
    ...
    it("filters by an explicit hubrpc.* interfaceId despite the default hiding", async ({ onTestFinished }) => {
        const d = createTestDisposableStore(onTestFinished);
        const gated = await makeGatedHub(d);
        const client = await makeGatedHarness(gated, d);

        // `hubrpc.*` is hidden by default, but filtering for an exact `hubrpc.*`
        // interfaceId opts back in — so every service exposing `hubrpc.directory`
        // (the connection root, the `hub` directory, and `hello`) is listed.
        const res = await client.callTool({ name: "runHubRpcScript", arguments: { code: `async ({ con }) => con.explore({ requestPermission: true, interfaceId: "hubrpc.directory" })` } });
    ...
    it("returns an empty listing when the filter matches nothing", async ({ onTestFinished }) => {
        ...
        const res = await client.callTool({ name: "runHubRpcScript", arguments: { code: `async ({ con }) => con.explore({ requestPermission: true, serviceId: "does-not-exist" })` } });
    ...
    it("pages through results with offset and nextOffset", async ({ onTestFinished }) => {
        ...
        const page1 = await client.callTool({ name: "runHubRpcScript", arguments: { code: `async ({ con }) => con.explore({ requestPermission: true, maxResults: 2 })` } });
        ...
        const page2 = await client.callTool({ name: "runHubRpcScript", arguments: { code: `async ({ con }) => con.explore({ requestPermission: true, maxResults: 2, offset: 2 })` } });
    ...
    it("includes hubrpc.* internal interfaces when showHubrpcInternalInterfaces is set", async ({ onTestFinished }) => {
        const d = createTestDisposableStore(onTestFinished);
        const gated = await makeGatedHub(d);
        const client = await makeGatedHarness(gated, d);

        // By default `hubrpc.*` reflection plumbing is hidden. With
        // `showHubrpcInternalInterfaces: true` the hello service's reflection
        // interfaces (hubrpc.directory / hubrpc.defaults / hubrpc.schemas) appear
        // alongside its `greeter` interface.
        const res = await client.callTool({ name: "runHubRpcScript", arguments: { code: `async ({ con }) => con.explore({ requestPermission: true, serviceId: "hello", showHubrpcInternalInterfaces: true })` } });
    ...
    it("includes the greeter interface's JSON schema when includeSchema is set", async ({ onTestFinished }) => {
        ...
        const res = await client.callTool({ name: "runHubRpcScript", arguments: { code: `async ({ con }) => con.explore({ requestPermission: true, serviceId: "hello", interfaceId: "greeter", includeSchema: true })` } });
    ...
    it("includes a generated TypeScript module when includeTypeScript is set", async ({ onTestFinished }) => {
        ...
        const res = await client.callTool({ name: "runHubRpcScript", arguments: { code: `async ({ con }) => con.explore({ requestPermission: true, serviceId: "hello", interfaceId: "greeter", includeTypeScript: true })` } });
                    ...
                    "entries": [
                      {
                        "interfaceId": "greeter",
                        "serviceId": "hello",
                        "typeScript": "import { defineInterface, notificationType, requestType } from "@vscode/hubrpc";
          import { z } from "zod";
    ...
    it("grep filters listings by a metadata substring", async ({ onTestFinished }) => {
        ...
        const res = await client.callTool({ name: "runHubRpcScript", arguments: { code: `async ({ con }) => con.explore({ requestPermission: true, grep: "greet" })` } });
    ...
    it("grep does not match terms that only appear inside the schema", async ({ onTestFinished }) => {
        ...
        const res = await client.callTool({ name: "runHubRpcScript", arguments: { code: `async ({ con }) => con.explore({ requestPermission: true, grep: "greeting" })` } });
    ...
    it("grepAllSchemas matches terms found inside an interface's JSON schema", async ({ onTestFinished }) => {
        ...
        const res = await client.callTool({ name: "runHubRpcScript", arguments: { code: `async ({ con }) => con.explore({ requestPermission: true, grepAllSchemas: "greeting" })` } });
