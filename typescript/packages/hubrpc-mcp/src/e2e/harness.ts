 ...
 * `HubRpcMcpServer` whose connection is a consumer-provided `HubSigningSender`
...
export async function makeUngatedHarness(d: TestDisposableStore): Promise<Client> {
    ...
    const server = new HubRpcMcpServer({
...
/**
 * Wire a `HubRpcMcpServer` whose provider attaches a **gated** link to the hub,
...
/**
 * Wire a `HubRpcMcpServer` whose **default connection** (no `connection` arg) is
