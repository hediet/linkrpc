...
import { ErrorCode, RpcError } from "@vscode/hubrpc";
import type { ResolvedEndpoint } from "@vscode/hubrpc/node";
import { z } from "zod";
...
export interface HubRpcMcpServerOptions {
    /**
     * Endpoint used when the tool call does not supply a `connection`. When
     * omitted, the server falls back to the `HUBRPC_ENDPOINT` / `HUBRPC_TOKEN`
...
const CONNECTION_SCHEMA = z.string().optional().describe(
    ...
    "`npipe://./pipe/vscode-hubrpc-…?token=…`, or `wss://host:port?token=…`. " +
    "The token may be embedded as a `token` query param or supplied via the " +
    "HUBRPC_TOKEN environment variable. When omitted entirely, the server falls " +
    "back to the HUBRPC_ENDPOINT and HUBRPC_TOKEN environment variables.",
);
...
const SERVER_NAME = "hubrpc-mcp";
const SERVER_VERSION = "0.0.1";

/**
 * MCP server exposing the `runHubRpcScript` tool plus task-management helpers.
 *
 ...
 * model can pull it in by issuing a one-line `runHubRpcScript` call.
 *
 ...
 */
export class HubRpcMcpServer {
    public static async startStdio(options?: HubRpcMcpServerOptions): Promise<HubRpcMcpServer> {
        const server = new HubRpcMcpServer(options);
    ...
    public constructor(options: HubRpcMcpServerOptions = {}) {
    ...
    private _registerTools(): void {
        this._mcp.registerTool(
            "runHubRpcScript",
            {
                ...
                description:
                    ...
                    "the HUBRPC_ENDPOINT / HUBRPC_TOKEN env vars.\n\n" +
                    ...
                    "`awaitHubRpcTask` to wait for it or `cancelHubRpcTask` to abandon " +
                    "it. Set `label` to give the task a " +
                    "clear debug name. Starting a new `runHubRpcScript` " +
                ...
                annotations: {
                    // HubRPC performs its own capability checks and consent flow, so MCP
            ...
            async (args) => {
                ...
                const host: SandboxHostApi = {
                    call: async (method, paramsJson, optsJson, onStreamMessage, signal) => {
                        // Most hubrpc schemas validate against `z.object({...})`, which
                    ...
                    explore: async (argsJson) => {
                        ...
                        // a single broad reflection grant (`hubrpc.*` on every service)
                    ...
                    requestAccess: async (argsJson) => {
                        ...
                        const result = await pooled.session.requestAccess({
                            consumer: { name: "hubrpc-mcp", purpose: req.purpose },
                ...
                try {
                    ...
                    if (outcome.status === "error") {
                        return _errorResult(
                            `runHubRpcScript failed: ${outcome.error}`,
                    ...
                    return _errorResult(
                        `runHubRpcScript failed: ${_describeError(e)}`,
                        traceEnabled ? trace : undefined,
                    );
                } finally {
                    disposeTrace();
        ...
        this._mcp.registerTool(
            "awaitHubRpcTask",
            {
                ...
                description:
                    "Wait up to `timeoutMs` for a background task started by `runHubRpcScript` " +
                ...
                inputSchema: {
                    taskId: z.string().describe("Task id returned by `runHubRpcScript`."),
        ...
        this._mcp.registerTool(
            "cancelHubRpcTask",
            {
                ...
                description:
                    "Soft-abort a background task started by `runHubRpcScript` and return its " +
...
async function _requestReflectionAccess(session: HubAccess): Promise<boolean> {
    const result = await session.requestAccess({
        consumer: { name: "hubrpc-mcp", purpose: "explore the hub (reflection)" },
        permissions: [
            {
                target: {
                    serviceId: { prefix: "" },
...
async function _autoRequestAccess(session: HubAccess, method: string): Promise<boolean> {
    const { serviceId, interfaceId, member } = _parseMethod(method);
    const result = await session.requestAccess({
        consumer: { name: "hubrpc-mcp", purpose: `invoke ${method}` },
        permissions: [{
            target: {
