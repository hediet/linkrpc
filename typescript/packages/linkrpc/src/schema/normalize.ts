import type { LinkRpcJsonSchema, DiscriminatorSchema } from "./linkRpcJsonSchema";
 ...
 */
export function normalizeJsonSchema(raw: unknown): LinkRpcJsonSchema {
    ...
    // Object: linkrpc requires `additionalProperties` to be set. Default to
    ...
    if (Array.isArray(out["oneOf"]) && !("discriminator" in out)) {
        const detected = _detectDiscriminator(out["oneOf"] as LinkRpcJsonSchema[]);
    ...
    return out as unknown as LinkRpcJsonSchema;
}
...
function normalizeChild(key: string, v: unknown): unknown {
    switch (key) {
        case "properties": {
            ...
            const out: Record<string, LinkRpcJsonSchema> = {};
 ...
 */
function _detectDiscriminator(branches: LinkRpcJsonSchema[]): DiscriminatorSchema | undefined {
    if (branches.length < 2) return undefined;
    // Each branch must be an object schema with `properties`.
    const branchPropsList: Record<string, LinkRpcJsonSchema>[] = [];
    for (const b of branches) {
        if (b === true || b === false) return undefined;
        if ((b as { type?: string }).type !== "object") return undefined;
        const props = (b as { properties?: Record<string, LinkRpcJsonSchema> }).properties;
...
function _constPropNames(props: Record<string, LinkRpcJsonSchema>): string[] {
    const out: string[] = [];
