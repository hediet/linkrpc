...
/**
 * A live hubrpc connection plus the underlying child / socket. The CLI
...
export interface CliConnection {
     ...
     * Hand this to `new HubRpcConnection(conn.rpcChannel)` when a command needs
     ...
     * `HubRpcConnection` from it binds the inbound handler, so do not also use
     ...
     * here, bypassing the typed {@link HubRpcConnection} dispatch. Pass
     * `undefined` to detach.
     */
