/**
 * Endpoint resolution for the CLI = "where the hubrpc server lives, and how to
 * reach (or start) it". The parsed truth is the {@link ResolvedEndpoint} union from
 * `@vscode/hubrpc/node`; this module turns the ergonomic flags / env vars into
 * one.
 *
 ...
 *   4. `HUBRPC_ENDPOINT` env var        → legacy bare path / ws url
 *
 * `--endpoint-token` overrides the token from a URI / `HUBRPC_TOKEN` for
...
export type { ResolvedEndpoint } from "@vscode/hubrpc/node";
...
export interface ResolveEndpointInput {
    ...
    /** `--endpoint-token <token>`: overrides the URI / `HUBRPC_TOKEN` token. */
