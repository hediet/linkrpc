...
describe('aggregating hubAccessManifest (scan the hub, merge, one approver, capability-gated)', () => {
    it('discovers manifests, merges them, approves via one approver, and routes decisions home', async () => {
        ...
        const localServer = HubRpcConnection.fromTransport(localPair.a);
        const localClientConn = HubRpcConnection.fromTransport(localPair.b);
        ...
        const aggMgrServer = HubRpcConnection.fromTransport(aggMgrPair.a);
        const aggMgrClient = HubRpcConnection.fromTransport(aggMgrPair.b);
...
function signedConnection(transport: IMessageTransport, principal: Principal): HubRpcConnection {
    const signed = SigningSender.wrapChannel(JsonRpcChannel.create(transport), { principal });
    return new HubRpcConnection(signed.sender);
}
