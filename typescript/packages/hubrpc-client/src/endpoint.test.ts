...
describe("resolveEndpoint", () => {
    ...
    it("ignores HUBRPC_TOKEN for an explicit --endpoint", () => {
        const r = resolveEndpoint({
            endpoint: "unix:/x.sock",
            env: { HUBRPC_TOKEN: "envtok" },
        });
        expect(r.endpoint).toEqual({ kind: "socket", path: "/x.sock" });
    });

    it("resolves a socket endpoint from env with HUBRPC_TOKEN", () => {
        const r = resolveEndpoint({
            env: { HUBRPC_ENDPOINT: "/tmp/x.sock", HUBRPC_TOKEN: "abc" },
        });
        expect(r.endpoint).toEqual({ kind: "socket", path: "/tmp/x.sock", token: "abc" });
    });

    it("env URI token wins over HUBRPC_TOKEN", () => {
        const r = resolveEndpoint({
            env: { HUBRPC_ENDPOINT: "unix:/x.sock?token=fromuri", HUBRPC_TOKEN: "fromenv" },
        });
        expect(r.endpoint).toEqual({ kind: "socket", path: "/x.sock", token: "fromuri" });
    });

    it("--endpoint-token overrides HUBRPC_TOKEN for an env endpoint", () => {
        const r = resolveEndpoint({
            endpointToken: "flag",
            env: { HUBRPC_ENDPOINT: "/x.sock", HUBRPC_TOKEN: "env" },
        });
    ...
    it("resolves a bare ws endpoint from env", () => {
        const r = resolveEndpoint({
            env: { HUBRPC_ENDPOINT: "wss://hub.example.com", HUBRPC_TOKEN: "abc" },
        });
