...
`runLinkRpcScript` uses `console.log` as the script-facing progress API. Logs are
stored on the sandbox task and returned by `runLinkRpcScript` or
`awaitLinkRpcTask`. Stream messages are logged automatically when a
...
- `runLinkRpcScript` can report logs produced during its foreground phase.
- Once `runLinkRpcScript` returns a parked task, its progress stream is over.
- `awaitLinkRpcTask` should report logs produced while that await request is
  active. A later await call uses its own progress token.

Do not keep `runLinkRpcScript` open indefinitely merely to preserve its progress
...
3. While `runLinkRpcScript` or `awaitLinkRpcTask` has an active progress token,
...
Add a cursor to `awaitLinkRpcTask`, for example:
...
- Do not let notification sends block the QuickJS pump or LinkRPC stream
  callback.
...
- Stop per-request progress subscriptions when the MCP request completes or is
  cancelled.
- Do not cancel the parked task merely because an `awaitLinkRpcTask` request is
...
- A foreground script log is delivered through progress and in the final tool
  result.
- A parked traffic watch delivers new logs during `awaitLinkRpcTask`.
