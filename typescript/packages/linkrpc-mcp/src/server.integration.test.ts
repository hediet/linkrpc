...
async function makeHarness(): Promise<Harness> {
    ...
    const server = new LinkRpcMcpServer({ pool });
...
describe("LinkRpcMcpServer task names (integration)", () => {
    ...
    it("awaitLinkRpcTask returns the completed result once the RPC resolves", async () => {
        const h = (active = await makeHarness());
