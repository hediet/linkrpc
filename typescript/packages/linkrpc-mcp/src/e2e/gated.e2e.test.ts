...
describe("LinkRpcMcpServer e2e (gated hub, capability checking enabled)", () => {
    it("denies the tool's forwarded call when the signer holds no capability", async ({ onTestFinished }) => {
        ...
        const res = await client.callTool({ name: "runLinkRpcScript", arguments: { code: `({ con }) => con.call("hello", "greeter", "hello", { name: "world" })` } });
        ...
        expect(llmResult(res)).toMatchInlineSnapshot(`
          {
            "content": [
              {
                "text": "runLinkRpcScript failed: Permission required for hello::greeter::hello — no capability is held for this call.
    ...
    it("con.grants() is empty before any access is negotiated", async ({ onTestFinished }) => {
        ...
        const res = await client.callTool({ name: "runLinkRpcScript", arguments: { code: `async ({ con }) => con.grants()` } });
    ...
    it("admits the call after the tool negotiates a greeter cap via hubAccess", async ({ onTestFinished }) => {
        ...
        const res = await client.callTool({ name: "runLinkRpcScript", arguments: { code: `async ({ con }) => {
        ...
        expect(out).toMatchInlineSnapshot(`
          {
            ...
            "result": {
              ...
              "grants": [
                {
                  "audience": "id:key:mx3wi1wn_P8F3Cd-s6dnch4dczAwXL47uHfF5tNpla8",
                  "issuer": "id:key:oeTtmG8LwI6ohMQF8HAJP_77U8CKg4OEppqx8DFlyX0",
                  "oneShot": false,
                  "permissions": [
                    {
                      "canDelegate": false,
    ...
    it("auto-negotiates the greeter cap when the call opts into requestPermission", async ({ onTestFinished }) => {
        ...
        const res = await client.callTool({ name: "runLinkRpcScript", arguments: { code: `({ con }) => con.call("hello", "greeter", "hello", { name: "world" }, { requestPermission: true })` } });
    ...
    it("includes a JSON-RPC wire trace when the managed default call opts into trace", async ({ onTestFinished }) => {
        ...
        const res = await client.callTool({ name: "runLinkRpcScript", arguments: { code: `({ con }) => con.call("hello", "greeter", "hello", { name: "world" }, { requestPermission: true })`, trace: true } });
