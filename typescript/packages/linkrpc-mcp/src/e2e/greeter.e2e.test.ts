...
describe("LinkRpcMcpServer e2e (real hub + signing sender)", () => {
    it("routes a runLinkRpcScript tool call through the hub to the greeter service", async ({ onTestFinished }) => {
    ...
    it("captures sandbox console output in the result logs", async ({ onTestFinished }) => {
        ...
        const res = await client.callTool({ name: "runLinkRpcScript", arguments: { code: `() => { console.log("hello from sandbox", 42); return 1; }` } });
    ...
    it("threads lastResultVal from one runLinkRpcScript call into the next", async ({ onTestFinished }) => {
        const d = createTestDisposableStore(onTestFinished);
        const client = await makeUngatedHarness(d);
        ...
        const first = await client.callTool({ name: "runLinkRpcScript", arguments: { code: `() => ({ n: 41 })` } });
        ...
        const second = await client.callTool({ name: "runLinkRpcScript", arguments: { code: `({ lastResultVal }) => lastResultVal.n + 1` } });
    ...
    it("surfaces a service error as an error result when calling an unknown member", async ({ onTestFinished }) => {
        ...
        // completed one.
        const res = await client.callTool({ name: "runLinkRpcScript", arguments: { code: `({ con }) => con.call("hello", "greeter", "goodbye", { name: "world" })` } });

        expect(llmResult(res)).toMatchInlineSnapshot(`
          {
            "content": [
              {
                "text": "runLinkRpcScript failed: Method not found: hello::greeter::goodbye
