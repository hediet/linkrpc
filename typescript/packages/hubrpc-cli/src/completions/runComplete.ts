 ...
 * `HUBRPC_ENDPOINT` fallback — the extension passes `{}` so completion targets
 * the endpoint named on the line rather than the extension host's environment.
 */
import { setupSigning } from '@vscode/hubrpc-client';
import { parsePrincipalSpec } from '@vscode/hubrpc-client';
import { connect } from '@vscode/hubrpc-client';
import { isHubEndpoint } from '@vscode/hubrpc/node';
import { resolveEndpoint, type ResolvedEndpoint } from '@vscode/hubrpc-client';
...
export interface CompleteForLineOptions {
    ...
    /**
     * Environment consulted for the `HUBRPC_ENDPOINT` fallback. Defaults to
