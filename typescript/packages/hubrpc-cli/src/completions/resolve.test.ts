...
describe('resolveSlot', () => {
    ...
    it('returns the variadic positional type past declared positionals', () => {
        const r = slotFor('hub node script.js arg1 ');
        expect(r.slot.kind).toBe('positional');
        if (r.slot.kind === 'positional') {
            expect(r.slot.type).toBe('free'); // variadic
            expect(r.slot.index).toBe(2);
        }
    });
