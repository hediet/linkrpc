...
describe("CLI commands against an in-process hubrpc server", () => {
    ...
    it("ls dump preserves directory reporters and deduplicates schemas by hash", async () => {
        const fx = makeFixture();
        const directory = await mkdtemp(path.join(tmpdir(), "hubrpc-ls-dump-"));
        ...
        try {
            ...
            expect(dump.format).toBe("hubrpc-hub-dump");
