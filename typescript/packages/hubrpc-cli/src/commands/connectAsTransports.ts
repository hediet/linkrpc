...
/** Spawn a `cmd-stdio` target and talk hubrpc over its stdin/stdout. */
export async function openStdioTransport(
    ...
    // stdout carries the hubrpc protocol; only stderr can be logged.
...
export async function openAcceptOneTransport(
    ...
    const child = spawnCommand(command, {
        ...
        env: { ...process.env, ...env, HUBRPC_ENDPOINT: socketPath, HUBRPC_TOKEN: token },
        ...(cwd !== undefined ? { cwd } : {}),
    });
