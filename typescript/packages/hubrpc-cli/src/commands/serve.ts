/**
 * `hubrpc serve <config>` — run a hub from a declarative `HubConfig` file and
 * block until interrupted. Thin CLI face over the shared `@vscode/hubrpc-hub`
 * engine, so one config format drives the CLI hub, the server hub, and the
 * VS Code extension.
 */
import { loadHubConfig, printHubConfigSchema } from '@vscode/hubrpc-hub/config-file';
...
export async function serveCommand(opts: ServeOptions): Promise<void> {
    const { serveHub } = await import('@vscode/hubrpc-hub/serve');
    await serveHub({
        configPath: opts.configPath,
        cmdInteractive: opts.cmdInteractive,
        logMessages: opts.logMessages,
        log: (line) => process.stderr.write(`hubrpc: ${line}\n`),
    });
}
