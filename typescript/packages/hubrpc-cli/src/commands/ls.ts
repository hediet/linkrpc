...
function renderLsState(state: LsState, opts: Pick<LsOptions, "interfaceId">): string {
    ...
    return [
        `HubRPC directory (${status}, revision ${state.revision})`,
        "",
...
export interface HubDump {
    readonly format: "hubrpc-hub-dump";
    readonly version: 2;
    readonly revision: number;
    readonly complete: boolean;
    ...
    readonly schemas: Readonly<Record<string, HubRpcInterfaceSchema>>;
...
interface DumpSchemaCache {
    readonly schemas: Map<string, HubRpcInterfaceSchema>;
