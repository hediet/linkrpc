...
export interface StaticHubSchemaDocument {
    ...
    interfaceSchemas: HubRpcInterfaceSchema[];
}
...
export function parseStaticHubSchema(value: unknown): StaticHubSchema {
    ...
    const schemasByKey = new Map<string, HubRpcInterfaceSchema>();
...
function parseInterfaceSchema(value: unknown, path: string): HubRpcInterfaceSchema {
    ...
    const typed = schema as unknown as HubRpcInterfaceSchema;
...
function requireResolvedReference(
    ...
    schemasByKey: ReadonlyMap<string, HubRpcInterfaceSchema>,
    path: string,
): void {
