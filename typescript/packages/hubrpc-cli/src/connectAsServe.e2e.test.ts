...
import { z } from 'zod';
import { connectionTokenBinderInterface } from '@vscode/hubrpc-hub/hub/server';
import { SocketServer } from '@vscode/hubrpc-hub/hub/server/node';
import { runHub, type RunningHub } from './engine/runHub';
import { parseHubConfig } from '@vscode/hubrpc-client';
...
describe('connect-as e2e (runHub config path: bound listener + central connectionTokenBinder)', () => {
    it('serves the hub from config, mints via hub::connectionTokenBinder, and splices a target as its slot', async () => {
        ...
        try {
            ...
            const consumer = HubRpcConnection.fromTransport(consumerPair.b);
