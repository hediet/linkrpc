 ...
 * `@vscode/observables` and `@vscode/hubrpc-node-runner`, which are bundled
 * into the output: they are dev-only dependencies (the latter is also private
 * and exposes its interface as TypeScript source), so they must not be
...
const external = (id) => {
    ...
    if (id === '@vscode/hubrpc-node-runner' || id.startsWith('@vscode/hubrpc-node-runner/')) {
        return false;
    }
    ...
    if (id === '@vscode/hubrpc-client' || id.startsWith('@vscode/hubrpc-client/')) {
        return false;
    }
    if (
        !id.startsWith('.') &&
...
const plugins = () => [
    ...
    typescript({
        ...
        // The script-runner interface is bundled from its TypeScript source
        // (it lives in a private, source-only workspace package), so the
        // plugin must transform it alongside our own `src` files.
        include: [
            'src/**/*.ts',
            'src/**/*.tsx',
            '../hubrpc-node-runner/src/scriptRunnerInterface.ts',
            '../hubrpc-client/src/**/*.ts',
        ],
    }),
];
