//! M6 connector integration: drive [`connect_to_hub`](hubrpc_tokio::connect_to_hub) end-to-end over
//! both transports — a Unix-domain socket (with `hello` preamble) and a WebSocket (token in the
//! `Authorization` header). Both wrap the same `HubRpcConnection`, so a single helper serves the
...
use hubrpc::prelude::*;
use hubrpc_tokio::{connect_to_hub, ConnectOptions, WebSocketTransport};
...
fn subset_of<T: JsonSchema>() -> Value {
    hubrpc::schema::schemars_to_subset(&serde_json::to_value(schema_for!(T)).unwrap())
...
fn serve(transport: Box<dyn MessageTransport>) -> Arc<HubRpcConnection> {
    let conn = Arc::new(HubRpcConnection::new(transport));
    conn.register(
...
async fn assert_order(client: &HubRpcConnection) {
...
async fn connect_to_hub_over_unix_socket_with_preamble() {
    ...
    let path = dir.join(format!("hubrpc-m6-{}.sock", std::process::id()));
    let _ = std::fs::remove_file(&path);
    let listener = hubrpc_tokio::unix::UnixHubListener::bind(&path).unwrap();
    ...
    let client = connect_to_hub(ConnectOptions {
        ...
        hubrpc_token: Some(TOKEN.to_string()),
        ..Default::default()
    })
    .await
