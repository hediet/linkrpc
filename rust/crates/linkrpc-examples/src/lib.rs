//! Runnable linkrpc examples sharing one `#[link_rpc_interface]` contract per example.
//!
...
/// [`format_endpoint_uri`](linkrpc::prelude::format_endpoint_uri) turns the result into the matching
/// `unix:` / `npipe://` URI, and [`HubListener::bind`](linkrpc_tokio::HubListener::bind) accepts it
...
pub fn unique_endpoint_address(label: &str) -> String {
    ...
    #[cfg(windows)]
    {
        format!(r"\\.\pipe\linkrpc-{label}-{pid}-{nanos}")
    }
    #[cfg(not(windows))]
    {
        std::env::temp_dir()
            .join(format!("linkrpc-{label}-{pid}-{nanos}.sock"))
            .to_string_lossy()
            .into_owned()
    }
}
