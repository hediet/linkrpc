...
///
/// `schema` is the JSON object form of a [`LinkRpcInterfaceSchema`]. The top-level
...
/// Compute the interface hash for a typed [`LinkRpcInterfaceSchema`].
pub fn compute_interface_hash(schema: &LinkRpcInterfaceSchema) -> String {
    let value = serde_json::to_value(schema).expect("schema serialization is infallible");
    compute_interface_hash_value(&value)
}

/// Project the interface's JSON-Schema positions onto the decidable linkrpc
