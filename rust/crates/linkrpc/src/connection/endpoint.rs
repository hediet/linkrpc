...
//! (`linkrpc/src/connection/endpointUri.ts`).
//!
//! [`parse_endpoint_uri`] resolves a `LINKRPC_ENDPOINT` string into a [`ResolvedEndpoint`];
...
/// A resolved `LINKRPC_ENDPOINT`, mirroring the TS `ResolvedEndpoint` union.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ResolvedEndpoint {
    ...
    /// Start a local hub and inject `LINKRPC_ENDPOINT`/`LINKRPC_TOKEN` into the child.
    CmdEnv {
        /// Command to spawn.
        command: EndpointCommand,
...
/// Parse a `LINKRPC_ENDPOINT` URI into a [`ResolvedEndpoint`].
