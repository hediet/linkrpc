...
pub struct RawStreamingCall {
    ...
    pub(super) result: oneshot::Receiver<Result<JsonValue, JsonRpcError>>,
        ...
        R: DeserializeOwned + Send + 'static,
    {
        ...
        let call_result = CallResult {
            inner: Box::pin(async move {
                let value = result.await.map_err(|_| disconnected())??;
...
struct StreamInner {
    ...
    result_sender: Option<oneshot::Sender<Result<JsonValue, JsonRpcError>>>,
...
impl StreamState {
    pub(super) fn outbound(
        ...
        Arc<Self>,
        oneshot::Receiver<Result<JsonValue, JsonRpcError>>,
    ) {
    ...
    pub(super) fn push_payload(&self, value: JsonValue) -> Result<(), JsonRpcError> {
        ...
        if inner.incoming.len() >= STREAM_BUFFER_CAPACITY {
            ...
            drop(inner);
            self.complete_cleanup(cleanup, Some(Err(error.clone())));
            return Err(error);
        }
    ...
    pub(super) fn settle(&self, result: Option<Result<JsonValue, JsonRpcError>>) {
        ...
        let cleanup = Self::terminal_cleanup_locked(&mut inner, false);
        drop(inner);
        self.complete_cleanup(cleanup, result);
    }
    ...
    pub(super) fn disconnect(&self) {
        self.settle(Some(Err(disconnected())));
    }
    ...
    fn complete_cleanup(
        ...
        result: Option<Result<JsonValue, JsonRpcError>>,
    ) {
