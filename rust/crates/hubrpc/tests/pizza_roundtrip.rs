//! End-to-end `HubRpcConnection` round-trip over the in-memory transport pair.
//!
...
fn make_pair(cancelled: Arc<Mutex<Vec<String>>>) -> (HubRpcConnection, HubRpcConnection) {
    let (a, b) = transport_pair();
    let client = HubRpcConnection::new(Box::new(a));
    let server = HubRpcConnection::new(Box::new(b));
...
async fn duplicate_registration_is_rejected() {
    ...
    let conn = HubRpcConnection::new(Box::new(b));
    let iface = Arc::new(pizza_interface());
