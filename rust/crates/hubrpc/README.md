...
hubrpc protocol (normative spec: `hubrpc-spec` in the `@vscode/hubrpc` repository).

hubrpc is a **specialization of [JSON-RPC 2.0](https://www.jsonrpc.org/specification)**: every
message on the wire is a valid JSON-RPC message, and hubrpc adds just enough on top to make many
...
You write a plain Rust trait. The `#[hub_rpc_interface]` macro turns it into a server adapter, a
...
// in-memory pair for tests; real transports live in `hubrpc-tokio`
let (a, b) = transport_pair();
let client_conn = HubRpcConnection::new(Box::new(a));
let server_conn = HubRpcConnection::new(Box::new(b));
...
See [`crates/hubrpc-tokio/tests/macro_pizza.rs`](../hubrpc-tokio/tests/macro_pizza.rs) for the full
runnable example.
...
The full normative protocol lives in `hubrpc-spec` (in the `@vscode/hubrpc` repository).

## The crates

| Crate | Contents |
|---|---|
| **`hubrpc`** (this crate) | protocol core, schema/hashing, the connection runtime, reflection |
| **`hubrpc-macros`** | the `#[hub_rpc_interface]` procedural macro |
| **`hubrpc-tokio`** | Tokio transports (NDJSON over Unix sockets) |

## Status

This is a port-in-progress of the TypeScript `@vscode/hubrpc` framework. The non-streaming **wire
core, schema hashing, the connection runtime, reflection, and the `#[hub_rpc_interface]` macro are
