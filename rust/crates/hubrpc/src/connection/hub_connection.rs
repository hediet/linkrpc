//! `HubRpcConnection`: method-name routing + a live interface registry on top of a [`Channel`].
//!
//! Layers the hubrpc `::` addressing grammar and an interface registry over a plain JSON-RPC
...
//! interface also lets you opt into the three reflection interfaces via [`HubRpcConnection::enable_reflection`].
//!
//! Mirrors TS `connection/hubRpcConnection.ts` (non-streaming subset for this milestone).
...
/// Options for [`HubRpcConnection::register`].
...
/// A high-level hubrpc connection.
#[derive(Clone)]
pub struct HubRpcConnection {
    channel: Channel,
...
impl HubRpcConnection {
    ...
    pub fn new(transport: Box<dyn MessageTransport>) -> Self {
        ...
        HubRpcConnection { channel, inner }
    }
...
/// A directory listing entry (see [`HubRpcConnection::list_registered`]).
#[derive(Debug, Clone)]
...
/// macro-generated `*Server` adapters in [`HubRpcConnection::enable_reflection`].
struct Reflection {
    inner: Arc<RegistryInner>,
}
