...
//!
//! - `HUBRPC_ENDPOINT` — a `unix:/path` or `npipe://./pipe/name` URI naming the hub to dial, and
//! - `HUBRPC_TOKEN`    — the auth token written in the `hello` preamble on connect.
//!
//! That's the standard `HUBRPC_ENDPOINT` / `HUBRPC_TOKEN` contract a hub uses to hand a child
//! process its connection details.
//! [`connect_to_env_endpoint`](hubrpc_tokio::connect_to_env_endpoint) reads the env, dials (picking
...
//! `examples/calc_endpoint_client.rs` and `tests/endpoint.rs`. hubrpc is bidirectional, so the side
