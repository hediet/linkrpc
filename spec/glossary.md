...
| **Reserved key** | A `$hubrpc`-prefixed `params` key owned by linkrpc (`$hubrpc`, `$hubrpcSignature`, `$hubrpcUnsigned`). | 01 §3 |
...
| **Reflection** | The built-in `hubrpc.directory` / `.schemas` / `.defaults` interfaces. | 05, 09 §1 |
| **Connection-root directory** | The `hubrpc.directory` selected by an interface-form directory call on one connection. | 09 §2 |
| **Addressed directory** | A `hubrpc.directory` reached in fully-qualified form at a service id. | 09 §2 |
| **Directory referral** | A directory listing whose interface is `hubrpc.directory` and whose service id is the next walk target. | 09 §2 |
...
| **Signing domain value** | The wrapper key `hubrpc-sig/v1/<domain>`. | 06 §2 |
