...
Signing in hubrpc follows one rule, applied to any JSON object in a **domain**. This chapter uses `call`; chapter 07 uses `capability`. The identity extension adds domains such as `keyRotation`; the domain set is open.
...
A call's signing metadata lives in its `params.$hubrpc` object (chapter 01 §3.1). The partial interface below shows the fields relevant to identity; a message may contain additional JSON-RPC and hubrpc fields specified in earlier chapters.
